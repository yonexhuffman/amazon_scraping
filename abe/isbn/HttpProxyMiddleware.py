...
class HttpProxyMiddleware(object):
    ...
    def fetch_new_proxyes(self):
        ...
        new_proxyes = fetch_free_proxies.fetch_all(https=self.use_https)
    ...
    def inc_proxy_index(self, current=-1):
        ...
        if self.proxy_index == 0 and datetime.now() < self.last_no_proxy_time + timedelta(minutes=10):
    ...
    def set_proxy(self, request):
        ...
        if 'proxy_index' in request.meta:
            self.proxy_index = request.meta['proxy_index']
        else:
    ...
    def process_request(self, request, spider):
        ...
        logger.info('before set proxy')
        self.set_proxy(request)
        logger.info('after set proxy')
